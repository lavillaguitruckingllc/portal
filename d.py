...
<div class="card-header bg-dark text-white text-center py-4"><h4 class="mb-0">Broker Portal Login</h4></div><div class="card-body p-4">
...
<button class="btn btn-dark w-100 py-2 fs-5">Access Account</button></form><hr><p class="text-center mb-0">New Brokerage? <a href="/register" class="text-primary fw-bold">Become an Approved Vendor</a></p></div></div></div></div>'''

HTML_REGISTER = '''<div class="row justify-content-center"><div class="col-md-8"><div class="card shadow-lg">
<div class="card-header bg-dark text-white py-3"><h4 class="mb-0"><i class="bi bi-building-add"></i> Brokerage Setup Application</h4></div><div class="card-body p-4">
...
def admin_brokers():
    if session.get('role') != 'admin': return redirect(url_for('index'))
    with get_db_connection() as conn:
        carriers = conn.cursor().execute('''SELECT u.*, 
            ...
            FROM users u WHERE u.role='supplier' ORDER BY u.company_name ASC''').fetchall()
    return render_full(HTML_BROKERS_DIRECTORY, carriers=carriers)
